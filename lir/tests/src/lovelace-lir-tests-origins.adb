...
package body Lovelace.Lir.Tests.Origins is
   ...
   procedure Test_Codecs_Drop_Origins (The_Test : in out Fixture) is
      pragma Unreferenced (The_Test);
      Name_Span      : constant Lovelace.Common.Source.Source_Span :=
        (First => (Byte_Index => 1, Line => 1, Column => 1), Last => (Byte_Index => 5, Line => 1, Column => 5));
      Unit_Span      : constant Lovelace.Common.Source.Source_Span :=
        (First => (Byte_Index => 1, Line => 1, Column => 1), Last => (Byte_Index => 20, Line => 2, Column => 1));
      ...
      The_Subroutine : Subroutines.Subroutine :=
        Support.Make_Subroutine
          (Name => "Hello", Return_Type => Types.Unit, Flags => Subroutines.Export_Flag or Subroutines.Entrypoint_Flag);
      Decoded        : Modules.Module;
      Decoded_Sub    : Subroutines.Subroutine;
   begin
      Modules.Set_Origin
        (The_Module,
         (Name_Span => Name_Span, Span => Unit_Span, Filename => Lovelace.Common.Source.Some_Filename (Holder)));
      Subroutines.Set_Origin
        (The_Subroutine, (Name_Span => Name_Span, Filename => Lovelace.Common.Source.Some_Filename (Holder)));
      Modules.Append_Subroutine (The_Module, The_Subroutine);

      Decoded := Support.Must_Decode (Support.Must_Encode (The_Module, "encode with origins"), "decode with origins");

      AUnit.Assertions.Assert
        (Condition => not Modules.Origin (Decoded).Present, Message => "decoded module origin absent");

      Decoded_Sub := Modules.Get_Subroutine (Decoded, 1);
      AUnit.Assertions.Assert
        (Condition => not Subroutines.Origin (Decoded_Sub).Present, Message => "decoded subroutine origin absent");
   end Test_Codecs_Drop_Origins;
