...
todos:
  ...
  - id: "4"
    ...
    status: completed
  - id: "5"
    content: "5. Implement Lovelace.Compiler.Ast (module, subroutine, empty statements, Boolean flags, spans)."
    status: pending
  - id: "6"
...
- **AST shape:** root module named `IDENTIFIER`; module contains one subroutine also named `IDENTIFIER`; empty body; return type **unit**; `Is_Entrypoint` and `Is_Export` both **True**.
...
Distinct names from LIR (`Module` / `Subroutine` live under `Lovelace.Compiler.Ast`, never `with Lovelace.Lir.*`).

- **`Statement_Sequence`**: private empty vector wrapper (length 0 only this slice); ready for future statements.
- **`Subroutine`**: name (UTF-8 `Unbounded_String` from identifier lexeme), `Name_Span`, `Filename`, `Is_Entrypoint`, `Is_Export` (**Boolean** flags, not `mod 2**32`), `Return_Type` (`Types.Type_Expression`), `Body` (`Statement_Sequence`).
...
Accessors for tests and later IR gen: name, flags, return type, subroutine count/element, body length, spans.
...
- On success: build module + one subroutine (same name), `Is_Entrypoint`/`Is_Export` True, `Return_Type` = `Unit_Type`, empty body, spans from tokens.
...
- Success assertions: module name, one subroutine, same name, both flags True, return `Unit`, body length 0, module span covers program…`.`
...
5. Implement `Lovelace.Compiler.Ast` (module, subroutine, empty statements, Boolean flags, spans).
